...
const SHARED_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Bloco de assinatura manual (quando não há assinatura digital) */
  .sig-blank-box {
  ...
  .sig-blank-rows { display: grid; grid-template-columns: 1fr 1fr; gap: 20pt; margin-top: 8pt; }
  .sig-blank-field { display: flex; flex-direction: column; gap: 36pt; }
  .sig-blank-line {
    border-bottom: 1pt solid #334155;
    width: 100%;
  }
  .sig-blank-label { font-size: 7pt; text-transform: uppercase; letter-spacing: 1.5px; color: #94a3b8; margin-top: 4pt; }
  .sig-consent {
    margin-top: 12pt;
    font-size: 7.5pt;
    color: #64748b;
  ...
  const sigBlankHtml = `
    <div class="sig-blank-box">
      <p class="sig-blank-instruction">
        Ao assinar abaixo, o representante do cliente declara que os serviços descritos neste
        documento foram realizados conforme especificado, em conformidade com o contrato vigente.
      </p>
      <div class="sig-blank-rows">
        <div>
          <div class="sig-blank-field">
            <div class="sig-blank-line"></div>
          </div>
          <div class="sig-blank-label">Assinatura do cliente</div>
        </div>
        <div>
          <div class="sig-blank-field">
            <div class="sig-blank-line"></div>
          </div>
          <div class="sig-blank-label">Nome legível</div>
        </div>
        <div>
          <div class="sig-blank-field">
            <div class="sig-blank-line"></div>
          </div>
          <div class="sig-blank-label">Cargo / Empresa</div>
        </div>
        <div>
          <div class="sig-blank-field">
            <div class="sig-blank-line"></div>
      ...
      <p class="sig-consent">
        ...
        Este documento tem validade jurídica como assinatura eletrônica nos termos da Lei 14.063/2020 (MP 2.200-2/2001)
        quando assinado digitalmente via plataforma, ou como registro físico quando assinado manualmente.
      </p>
    </div>`;

  const sigHtml = signature ? sigDigitalHtml : sigBlankHtml;
  ...
  <div class="page">
    ...
    <div class="body">
      ...
      <div class="fields">
        ...
        ${field('Início', fmtDate(activity.started_at))}
        ${field('Supervisor', activity.supervisor_name ?? '')}
        ${field('Cliente', activity.client_name ?? '')}
        ${activity.ended_at ? field('Término', fmtDate(activity.ended_at)) : ''}
      </div>
